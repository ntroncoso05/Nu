...
void Sandbox2D::OnAttach()
{
	...
	m_SpriteSheet = Nu::Texture2D::Create("assets/game/textures/RPGpack_sheet_2X.png");

	m_TextureStairs = Nu::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 7.0f, 6.0f }, { 128.0f, 128.0f });
	m_TextureBarrel = Nu::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 8.0f, 2.0f }, { 128.0f, 128.0f });
	m_TextureTree = Nu::SubTexture2D::CreateFromCoords(m_SpriteSheet, { 2.0f, 1.0f }, { 128.0f, 128.0f }, { 1.0f, 2.0f });
...
void Sandbox2D::OnUpdate(Nu::Timestep ts)
{
	...
	if (Nu::Input::IsMouseButtonPressed(NU_MOUSE_BUTTON_LEFT))
	{
		auto [x, y] = Nu::Input::GetMousePosition();
		auto width = Nu::Application::Get().GetWindow().GetWidth();
		auto height = Nu::Application::Get().GetWindow().GetHeight();
	...
	Nu::Renderer2D::BeginScene(m_CameraController.GetCamera());
	Nu::Renderer2D::DrawQuad({ 0.0f, 0.0f, 0.5f }, { 1.0f, 1.0f }, m_TextureStairs);
	Nu::Renderer2D::DrawQuad({ 1.0f, 0.0f, 0.5f }, { 1.0f, 1.0f }, m_TextureBarrel);
	Nu::Renderer2D::DrawQuad({ -1.0f, 0.0f, 0.5f }, { 1.0f, 2.0f }, m_TextureTree);
	Nu::Renderer2D::EndScene();
}
